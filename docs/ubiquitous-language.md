...
| Resource Planner | Ressourceplanlægger | (rapport) | Rapport der giver overblik over medarbejderes projekt-arbejdsbyrde og lader brugeren planlægge hvornår arbejdet skal udføres. Arbejder ud fra allokerede timer. |
| Allocation | Allokering | `Budget (h)` på opgave | Timer tildelt en medarbejder til specifikt arbejde på en projektopgave (allokering af timer). Føder Ressourceplanlæggeren. _Brug:_ "allokere", "allokering af timer". |
| Booking | Booking | `BookWorkload` (`workload`) | Manuel timetildeling, eller timer fanget fra en Outlook-aftale. Adskilt fra Allokering. |
