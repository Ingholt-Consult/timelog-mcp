...
// Required fields per the empirical gate (2026-06-16): ProjectID (validate 500s
// without a valid one), TaskName, BudgetHours, BudgetAmount, HourlyRateID,
// StartDate, EndDate. TaskTypeID and ParentTaskID are optional.
export const createTaskShape = {
  ...modeField,
  ProjectID: z.number().int().describe("ProjectID the task belongs to. Required (validate errors with a 500 if missing/invalid)."),
  ParentTaskID: z
    .number()
    .int()
    .optional()
  ...
  BudgetHours: z.number().describe("Budgeted hours. Required."),
  BudgetAmount: z.number().describe("Budgeted amount. Required."),
  HourlyRateID: z.number().int().describe("HourlyRateID for the budget (see list_contract_hourly_rates). Required."),
...
// Fields shared by both contract create models. Required per the gate
// (2026-06-16): ProjectID, ContractName, ContractStatus.
const contractCommon = {
  ...modeField,
  ProjectID: z.number().int().describe("ProjectID the contract belongs to. Required and must be > 0."),
  ContractName: z.string().describe("Contract name. Required."),
  ContractStatus: z
    .number()
    .int()
    .min(1)
    .max(4)
    .describe("ContractStatus enum: 1=Quotation, 2=Active, 3=Completed, 4=Cancelled (confirmed via the gate, 2026-06-16; 0 is invalid). Required."),
...
// POST /payment — PaymentApiCreateModel. A payment is a payment-plan line on a
// contract. Required per the gate (2026-06-16): ProjectID, ProjectSubContractID,
// Name, InvoiceDate. The contract MUST be a fixed-price contract (a T&M contract
// rejects payments).
export const createPaymentShape = {
  ...modeField,
  ProjectID: z.number().int().describe("ProjectID the payment belongs to. Required."),
  ProjectSubContractID: z.number().int().describe("ContractID the payment is on (see list_contracts). Required, and must be a fixed-price contract."),
  TaskID: z.number().int().optional().describe("TaskID the payment is tied to (per-task revenue recognition)."),
  Name: z.string().describe("Payment name. Required."),
  Comment: z.string().optional().describe("Comment."),
  Amount: z.number().optional().describe("Payment amount."),
  InvoiceDate: z.string().describe("Invoice date, ISO 8601. Required."),
